xml.rss( :version => '2.0', 'xmlns:atom' => 'http://www.w3.org/2005/Atom' ) do
	xml.channel do
		xml.title( "#{@committee.name} - #{@publication_type.name}" )
		xml.description( "Updates whenever a there's a new #{@publication_type.name}." )
		xml.link( committee_publication_type_show_url )
		...
		xml.tag!( 'atom:link', { :href => committee_publication_type_show_url( :format => 'rss' ), :rel => 'self', :type => 'application/rss+xml' } )
